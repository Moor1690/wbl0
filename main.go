...
func NATSfoo(orders *map[string]Order) {
	...
	_, err = nc.Subscribe(subject, func(msg *stan.Msg) {
		fmt.Printf("Получено из %s: %s\n", subject, msg.Data)
		orderFromNats := string(msg.Data)
		insertToDB(&orderFromNats)
		postgresfoo(orders)
	})
	if err != nil {
		log.Fatal(err)
...
func postgresfoo(orders *map[string]Order) {
	ctx := context.Background()

	// Подключениек postgres
	config, err := pgxpool.ParseConfig("postgres://user:0000@localhost:5432/wb")
	if err != nil {
	...
	*orders, err = GetAllOrders(ctx, pool)
	if err != nil {
